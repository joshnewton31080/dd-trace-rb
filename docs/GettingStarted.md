...
`ddtrace` will submit traces to the agent via Unix Domain Socket (UDS) by default (if a socket is discovered at `/var/run/datadog/apm.socket`). Otherwise, it will fallback and submit traces via HTTP to `127.0.0.1:8126`.

If your Datadog Agent is listening at either location, no further configuration should be required.

If your agent runs on a different host or container than your application, you will need to configure your application to send trace data to it.
